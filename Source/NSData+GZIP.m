...
@implementation NSData (svga_GZIP)
...
- (NSData *)svga_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self svga_isGzippedData])
    {
        return self;
    }
...
- (NSData *)svga_gzippedData
{
    return [self svga_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)svga_gunzippedData
{
    if (self.length == 0 || ![self svga_isGzippedData])
    {
        return self;
